...
Pair* helper(TreeNode<int>* root){
    ...
    for(int i = 0; i < root -> children.size(); i++){
        ...
        if(smallAns -> largest -> data > ans -> largest -> data){
            TreeNode<int>* temp = ans -> largest;
            ans -> largest = smallAns -> largest;
            if(smallAns -> secondLargest != NULL){
                if(temp -> data < smallAns -> secondLargest -> data)
                    ans -> secondLargest = smallAns -> secondLargest;
                else if(temp -> data > smallAns -> secondLargest -> data)
                    ans -> secondLargest = temp;
            } else
                ans -> secondLargest = temp;
        } else if(smallAns -> largest -> data != ans -> largest -> data && (ans -> secondLargest == NULL || smallAns -> largest -> data > ans -> secondLargest -> data))
