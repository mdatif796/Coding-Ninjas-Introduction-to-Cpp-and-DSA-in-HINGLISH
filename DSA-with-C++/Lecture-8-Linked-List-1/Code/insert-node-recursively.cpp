...
Node* insertNode(Node *head, int i, int data) {
    ...
    if(temp == NULL)
        return head;
    // if we have to insert at 0th position 
    if(i == 0){
        newNode -> next = temp;
        temp = newNode;
        return temp;
    }
    // calling recursion from second node
    Node *tempHead = insertNode(temp -> next, i - 1, data);
    // connecting our head to recursive function head
    temp -> next = tempHead;
    return temp;
}
